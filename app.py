...
with tab2:
    ...
    # Path to your Project 2 match data file
    # (Adjust this path if your CSV is in a different folder!)
    csv_path = r'Z:\Python\project_2\overwatch_data.csv'
    
    try:
        df_ow = pd.read_csv(csv_path)
        
        # Display raw data insights directly from your file
        col1, col2 = st.columns(2)
        with col1:
            st.dataframe(df_ow, use_container_width=True)
        ...
        with col2:
            ...
            # Create a live chart based on your real CSV columns
            # (Assuming you have columns like 'Match_Result' or 'Win')
            if 'Result' in df_ow.columns:
                fig_ow = px.histogram(df_ow, x='Result', color='Result', title="Actual Win/Loss Distribution")
                st.plotly_chart(fig_ow, use_container_width=True)
                
    except Exception as e:
        st.warning(f"Note: To load your real match data, ensure your CSV is saved at {csv_path}. (Showing logic preview below)")
        
        # Keep the backup chart visible if the file path isn't set up yet
